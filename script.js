...
async function fetchStudentsFromGoogleSheetCSV() {
  ...
  return Array.from(studentMap.values()).map(finalizeStudent);
}
...
function mergeStudentData(existingStudent, incomingRow) {
  ...
  if (incomingRow.paperType === "CSAT" && incomingRow.paperData) {
    ...
    if (incomingRow.airValue !== "") {
      ...
      if (!existingStudent.rank || existingStudent.rank === "-") {
        existingStudent.rank = incomingRow.airValue;
      }
    }
...
function normalizeStudentRow(row, sheetInfo = {}) {
  ...
  const correct = normalizeNumber(
    getField(row, ["Correct", "Paper I Correct", "GS Correct", "Paper II Correct", "CSAT Correct"])
  );

  const incorrect = normalizeNumber(
    getField(row, ["Incorrect", "Paper I Incorrect", "GS Incorrect", "Paper II Incorrect", "CSAT Incorrect"])
  );

  const blank = normalizeNumber(
    getField(row, ["Blank", "Paper I Blank", "GS Blank", "Paper II Blank", "CSAT Blank"])
  );

  const sheetScore = normalizeNumber(
    getField(row, ["Score", "Paper I Score", "GS Score", "Paper II Score", "CSAT Score"])
  );
  ...
  const hasScoreColumns =
    ...
    rowHasValue(getField(row, ["Paper II Correct"])) ||
    rowHasValue(getField(row, ["Paper II Incorrect"])) ||
    rowHasValue(getField(row, ["Paper II Blank"])) ||
    rowHasValue(getField(row, ["Paper II Score"])) ||
    rowHasValue(getField(row, ["CSAT Correct"])) ||
    rowHasValue(getField(row, ["CSAT Incorrect"])) ||
    rowHasValue(getField(row, ["CSAT Blank"])) ||
    rowHasValue(getField(row, ["CSAT Score"]));

  const derivedScore =
    hasScoreColumns ? (rowHasValue(getField(row, ["Score", "Paper I Score", "GS Score", "Paper II Score", "CSAT Score"]))
      ? sheetScore
      : calculateScore(correct, incorrect)) : 0;
...
function renderDashboard() {
  if (!loggedInStudent || !app) return;

  app.innerHTML = `
    <div class="dashboard">
      <aside class="sidebar">
        <div class="sidebar-logo-row">
          <img src="logos-40-years 1.png" class="sidebar-years-logo" alt="40 years" />
          <img src="SRIRAM's-IAS.png" class="sidebar-logo" alt="Sriram IAS" />
        </div>
...
function downloadAdmitCard() {
  ...
  <div class="print-sheet">
    ...
    <div class="print-logo-row">
      <img src="logos-40-years 1.png" class="print-years-logo" />
      <img src="SRIRAM's-IAS.png" class="print-main-logo" />
    </div>
